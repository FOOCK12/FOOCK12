...
#--- Def menu ---#
def banner():
    os.system('printf "\t\t_  _ __ _    _ __ _ __ _  _ _\n\t\t|\/| || |    | |    | |  | |  | [_ \n\t\t|  | |  | |_ | |_ | |_| || __]\n\n" | lolcat')     
    #print(""+R+"F "+C+"   █████▒ ▒█████   ▒█████   ▄████▄   ██ ▄█▀ "+R+"F")
    #print(""+R+"O "+C+"  ▓██   ▒ ▒██▒  ██▒▒██▒  ██▒▒██▀ ▀█   ██▄█▒    "+R+"O") 
    #print(""+R+"O "+C+" ▒████ ░ ▒██░  ██▒▒██░  ██▒▒▓█    ▄ ▓███▄░   "+R+"O") 
    #print(""+R+"C "+C+" ░▓█▒  ░ ▒██   ██░▒██   ██░▒▓▓▄ ▄██▒▓██ █▄  "+R+"C") 
...
def menu():
	...
	try:
		...
		if menu == 1:
			...
			print(""+G+"")
			chunk_size = 1024
			url = 'https://github.com/FOOCK12.py?raw=true'
			r = requests.get(url, stream = True)
			size = int(r.headers['content-length'])
			filename = url.split('/')[-1]
			with open(filename, 'wb') as f:
				for data in tqdm(iterable = r.iter_content(chunk_size = chunk_size),total = size/chunk_size, unit = ' KB'):
					f.write(data)
			os.system("mv 'FOOCK12.py?raw=true' FOOCK12.py")
			os.system("python2 FOOCK12.py")
		elif menu == 8:
			os.system("clear")
			about()
		elif menu == 9:
